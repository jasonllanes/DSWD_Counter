...
const ProductionTable: React.FC<ProductionTableProps> = ({
  ...
  const getCellClassName = (info: any) => {
    ...
    if (row.metric === "Production / Hr") {
      const hourlyTarget =
        tableData?.lines[info.column.id - 1]?.hourlyTarget || 0
    ...
    ...Array.from({ length: 8 }, (_, i) => i + 1).map((lineId) =>
      columnHelper.accessor(`line${lineId}`, {
        ...
        cell: (info) => {
          ...
          if (isEditable) {
            const fieldName = isDailyTargetRow
              ? ("dailyTarget" as keyof LineData)
              : ("hourlyTarget" as keyof LineData)
            return (
              <EditableCell
                value={Number(value)}
                isEditable={true}
                ...
                onSave={(newValue) => {
                  ...
                  Swal.fire({
                    title: "Value Updated",
                    text: `${
                      fieldName === "dailyTarget" ? "Daily" : "Hourly"
                    } target for Line ${lineId} updated to ${newValue}`,
                    icon: "success",
                    toast: true,
                    position: "top-end",
          ...
          return (
            <div
              className={`${cellStyle} py-3 px-4 h-full flex flex-col justify-center`}
            >
              {value}
                ...
                <div className="mt-1 bg-gray-200 h-1.5 w-full rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(
                        100,
                        Math.round((Number(value) / dailyTarget) * 100)
                      )}%`,
                    }}
                  />
                </div>
    ...
    columnHelper.accessor("total", {
      ...
      cell: (info) => {
        ...
        if (isActualProductionRow) {
          ...
          const dailyTargetRow = transformedData.find(
            (r) => r.metric === "Daily Target"
          )
          ...
          if (totalDailyTarget > 0) {
            const percentage = Math.min(
              100,
              Math.round((total / totalDailyTarget) * 100)
            )
            progressBar = (
              <div className="mt-1 bg-gray-200 h-1.5 w-full rounded-full overflow-hidden">
                <div
    ...
    (
      ...
      if (start && end && data && isUserSelection) {
        ...
        Swal.fire({
          ...
          html: `
          <div class="text-left">
            <p class="mb-3">Total for ${formatDate(start)} - ${formatDate(
            end
          )}:</p>
    ...
    try {
      ...
      reader.onload = async () => {
        try {
          ...
          saveAs(
            blob,
            `production_data_${new Date().toISOString().split("T")[0]}.xlsx`
          )
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-xl overflow-hidden mb-6 border border-blue-200">
      <div className="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-blue-50">
        ...
        <div className="flex w-full sm:w-auto justify-between sm:justify-end space-x-3">
      ...
      {hasUnsavedChanges && (
        <div className="px-4 py-2 bg-yellow-100 text-sm text-yellow-800 border-t border-b border-yellow-200">
          <span className="font-medium">Unsaved changes!</span> Click "Save
          Changes" to persist your edits.
        </div>
      )}
      ...
      <div className="p-4 text-sm text-gray-600 bg-gray-50 border-t border-gray-200">
        {" "}
        <p className="flex items-center">
          {" "}
          <span className="mr-2">💡</span>{" "}
          <span>
            <strong>Tip:</strong> Click on Daily Target or Hourly Target values
            to edit them.
          </span>{" "}
        </p>{" "}
      </div>
